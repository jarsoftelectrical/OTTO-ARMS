...
//-- 04 December 2015
//-- Original Authors:  Anita de Prado: ana.deprado@bq.com
...
//-----------------------------------------------------------------
//-- Jason Snow
//-- AUGUST 2018: modified for OTTO ARMS
//-- Jason Snow
//-----------------------------------------------------------------
...
*/
 ...
 // #define PIN_YL 2 //servo[0]
  ...
  //#define PIN_LA 6 //servo[4]
  //#define PIN_RA 7 //servo[5]
...
const char programID[]="ARM2"; //Each program will have a ID
...
void setup(){
  ...
  Otto.home();
 // Animation - A little moment of initial surprise
 //-----
  for(int i=0; i<2; i++){
      for (int i=0;i<4;i++){  
        Otto.putAnimationMouth(dreamMouth,i);
        delay(150);
      }
  }
...
void receiveServo(){  
    ...
    bool error = false;
    char *arg;
    int servo_YL,servo_YR,servo_RL,servo_RR,servo_LA,servo_RA;
    ...
    arg = SCmd.next(); 
    if (arg != NULL) { servo_LA=atoi(arg); }    // Converts a char string to an integer  
    else {error=true;}

    arg = SCmd.next(); 
    if (arg != NULL) { servo_RA=atoi(arg); }    // Converts a char string to an integer  
    else {error=true;}
    ...
    if(error==true){
      ...
      int servoPos[6]={servo_YL, servo_YR, servo_RL, servo_RR, servo_LA, servo_RA}; 
...
void OttoSleeping_withInterrupts(){
  ...
  int bedPos_0[6]={100, 80, 60, 120, 30, 150}; 
